...
class MemoQLAI(QLAI):
	...
	def _torch_get_q_value(self, state, action, set_internal_state=True):
		features = self.get_features_after_action(state, action)
		return self.learning_model(features, set_internal_state=set_internal_state)

	def get_q_value(self, state, action):
		return self._torch_get_q_value(state, action, set_internal_state=False).item()
	...
	def _update_weights_experience(self, exp_sample):
		...
		for exp in exp_sample:
			...
			next_val_seq = torch.tensor([self.compute_value_from_q_values(next_state) for next_state in exp.next_state])
	...
	def compute_value_from_q_values(self, state):
		...
		q_values_list = torch.cat([self._torch_get_q_value(state, a, set_internal_state=False) for a in legal_actions])
